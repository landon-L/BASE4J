...
public class SysDeptController extends AbstractController<ISysProvider> {
	...
	@ApiOperation(value = "查询所有部门名称和编号")
	@RequiresPermissions("sys.base.dept.read")
	@PutMapping(value = "/read/idAndNameList")
	public Object queryDeptIdAndName(ModelMap modelMap) {
		Parameter parameter = new Parameter(getService(), "queryDeptIdAndName");
		logger.info("{} execute queryDeptIdAndName start...", parameter.getNo());
		//？代表得到什么类型就是什么类型
		List<?> deptList = provider.execute(parameter).getList();
		logger.info("{} execute queryDeptIdAndName end.", parameter.getNo());
